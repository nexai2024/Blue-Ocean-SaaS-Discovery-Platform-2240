...
const { 
  FiTrendingUp, FiPlus, FiTarget, FiArrowUp, 
  FiArrowDown, FiX, FiCheck, FiLightbulb, FiArrowRight 
} = FiIcons;

function ValueCurveDesigner() {
  const [selectedOpportunity, setSelectedOpportunity] = useState('compliance-saas');
                  ...
                  <tr key={i}>
                    ...
                    <td className="p-4 text-center">
                      <input type="number" min="0" max="10" value={d.yourProduct} 
                        onChange={(e) => updateDimension(i, 'yourProduct', parseInt(e.target.value))}
                        className="w-12 text-center border rounded p-1" />
                    </td>
                    <td className="p-4 text-center">
                      <input type="number" min="0" max="10" value={d.comp1} 
                        onChange={(e) => updateDimension(i, 'comp1', parseInt(e.target.value))}
                        className="w-12 text-center border rounded p-1" />
                    </td>
                    <td className="p-4 text-center">
                      <input type="number" min="0" max="10" value={d.comp2} 
                        onChange={(e) => updateDimension(i, 'comp2', parseInt(e.target.value))}
                        className="w-12 text-center border rounded p-1" />
                    </td>
                    <td className="p-4 text-center">
        ...
        <div className="space-y-6">
          <div className="bg-blue-600 p-6 rounded-xl text-white shadow-lg">
            ...
            <p className="text-blue-100 text-sm mb-4">
              "You are over-delivering on AI Automation and Onboarding Speed. To maintain a Blue Ocean, you MUST eliminate Enterprise Integrations which add complexity that your niche doesn't value."
            </p>
          ...
          <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
            ...
            <div className="p-4 bg-slate-50 rounded-lg text-xs leading-relaxed italic text-slate-700">
              "For small law firms who struggle with manual compliance, our SaaS provides 100% automated AI monitoring, unlike Clio which requires manual data entry."
            </div>
